...
describe('GET /api/destinations/:id/places', () => {
  it('returns 200 with an empty array when no places exist', async () => {
    const res = await request(app).get(`/api/destinations/${destinationId}/places`);
    expect(res.status).toBe(200);
    expect(res.body.status).toBe('success');
  ...
  it('returns only places belonging to the requested destination', async () => {
    ...
    const decoy = await Destination.create({ ...sampleDestination, name: 'Decoy City' });
    await Place.create([
      { destinationId, name: 'Test Place A', description: 'desc', category: 'Landmark', image: 'http://img.com/a' },
      { destinationId, name: 'Test Place B', description: 'desc', category: 'Nature', image: 'http://img.com/b' },
      { destinationId: decoy._id, name: 'Test Place Decoy', description: 'desc', category: 'Museum', image: 'http://img.com/c' },
    ]);
...
describe('GET /api/destinations/:id/restaurants', () => {
  it('returns 200 with restaurants sorted by rating descending', async () => {
    await Restaurant.create([
      { destinationId, name: 'Test Restaurant Low', cuisine: 'Italian', priceLevel: '$', rating: 3 },
      { destinationId, name: 'Test Restaurant High', cuisine: 'French', priceLevel: '$$$', rating: 5 },
      { destinationId, name: 'Test Restaurant Mid', cuisine: 'Thai', priceLevel: '$$', rating: 4 },
    ]);

    const res = await request(app).get(`/api/destinations/${destinationId}/restaurants`);
    expect(res.status).toBe(200);
  ...
  it('returns 404 for a non-existent destination', async () => {
    const fakeId = new mongoose.Types.ObjectId().toString();
    const res = await request(app).get(`/api/destinations/${fakeId}/restaurants`);
    expect(res.status).toBe(404);
    expect(res.body.message).toBe('Destination not found');
  });
});
...
describe('GET /api/destinations/:id/stays', () => {
  it('returns 200 with stays sorted by rating descending', async () => {
    await PropertyStay.create([
      { destinationId, name: 'Test Stay Budget', priceRange: '$50-100', rating: 3, location: 'City' },
      { destinationId, name: 'Test Stay Luxury', priceRange: '$500-1000', rating: 5, location: 'Beach' },
    ]);

    const res = await request(app).get(`/api/destinations/${destinationId}/stays`);
    expect(res.status).toBe(200);
    expect(res.body.results).toBe(2);
